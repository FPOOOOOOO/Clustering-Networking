function test_throughput_DCAMM(s_mobility,s_input)
...
S(n+1).xd = sink.x;
S(n+1).yd = sink.y;
STATISTICS = repmat(struct('DEAD',0,'PACKETS_TO_BS',0,'PACKETS_TO_CH',0,'PACKETS_OUT_CH',0,'PACKETS_TO_NODES',0,'PACKETS_OUT_NODES',0,'CHNUMS',0,'T_Running',0,'T_Change',0,'E_Use',0),r,times);
...
for time = 1 : times
    ...
    for timeIndex = 1:1:length(v_t)
        ...
        for i=1:1:n
            if(S(i).E <= 0)                    %画出死亡节点
                ...
                STATISTICS(timeIndex,time).DEAD = STATISTICS(timeIndex,time).DEAD + 1;
            end
        ...
        %全死了就结束
        if(STATISTICS(timeIndex,time).DEAD == n)
            break;
        end
        %如果有节点死了，那么就记录第一个死亡的节点时的轮次
        if(STATISTICS(timeIndex,time).DEAD && ~FIRST_DEAD(time))
        ...
        for i = 1:1:n
            ...
            if(S(i).E > 0)                     %节点还有能量
                ...
                STATISTICS(timeIndex,time).PACKETS_OUT_NODES = STATISTICS(timeIndex,time).PACKETS_OUT_NODES + 1;                        %这时候还不知道是簇头还是节点，所以没法标记，全部算成节点，取消C的包统计
                %%当前节点要收下其他Gi - 1 + 1个的包;
        ...
        for i = 1 : 1 : n
            if(S(i).E > 0)
                ...
                if C(S(i).Task_ID).child_num < temp_childnum
                   C(S(i).Task_ID).child_num = C(S(i).Task_ID).child_num + 1;
        ...
        STATISTICS(timeIndex,time).T_Change = t_change_N + t_change_C;
        STATISTICS(timeIndex,time).E_Use = e_use;
        ...
        for i = 1 : 1 : n
            if(S(i).type == 'N' && S(i).E > 0)
                ...
                di = sqrt((C(S(i).Task_ID).xd - S(i).xd)^2 + (C(S(i).Task_ID).yd - S(i).yd)^2);
                blk = (checkLineIntersectSquare(S(i).xd,S(i).yd,C(S(i).Task_ID).xd,C(S(i).Task_ID).yd,s_mobility,s_input));
                STATISTICS(timeIndex,time).T_Running = STATISTICS(timeIndex,time).T_Running + di/V_ele * 1000 +  blk * randomNumber; %以ms为单位
            end
    ...
    assignin('base', 'S', S);
    assignin('base', 'C', C);
    save('./DCAMM_throughputR.mat', 'STATISTICS');
