function test_DEEC(s_mobility,s_input)
v_t = 0:s_input.TIME_STEP:s_input.SIMULATION_TIME;

%% 0、遮挡移动部分，包括遮挡函数、移动的位置-时间
%计算遮挡函数
...
a = 1;                                 %高能节点能量倍数(1+a)
popt = 0.1;                            %最佳簇头选取概率
Etotal = 0;                            %网络初始总能量Etotal = 0;%网络初始总能量


% 创建节点结构体、簇头结构体、记录数据结构体
S=repmat(struct('xd',0,'yd',0,'G',0','type','N','E',0,'E0',0,'ENERGY',0,'min_dis',0,'min_dis_cluster',0,'C',0,'T_Change',0),n+1,1);    %这里多了一个E0，代表初始的能量，后面有根据能量进行分配的概率
...
for time = 1 : times
    ...
    %DEEC专属参数
    d1 = 0.765*xm/2;% DtoBS
    K = sqrt(0.5*n/pi)*do*xm/d1^2;% 最佳簇首数
    d2 = xm/sqrt(2*pi*K);% DtoCH
    Eround = packetLength*(2*n*ETX+n*EDA+K*Emp*d1^4+n*Efs*d2^2);%理论上一轮消耗能量
    R = 1.5*round(Etotal/Eround);%网络理论运行轮数，因为实际与理论会有差距，这里要x1.5
    ...
    for timeIndex = 1:1:length(v_t)
        ...
        if(mod(t_HB_period,10) == 0) %10ms一次，进行.G的归零，和1/p一样，所以没问题
            ...
            E_ = abs(Etotal*(1-timeIndex/R)/n);%理论网络当前平均能量,超出理论轮次后E_会为负数
            ...
            for i = 1:1:n
                ...
                %节点遍历,处理簇头数据
                if(S(i).E > 0 && E_ > 0)                     %节点还有能量
                    temp_rand = rand;              %随机种子
                    ptemp = popt*n*S(i).E*S(i).E0/(Etotal*E_);

                    if((S(i).G) <= 0)
                        if(temp_rand <= (ptemp/(1-ptemp*mod(timeIndex,round(1/ptemp)))))  %普通节点选择成为簇头
                            STATISTICS(timeIndex).CHNUMS = STATISTICS(timeIndex).CHNUMS + 1;
                            cluster = STATISTICS(timeIndex).CHNUMS;
                            S(i).type = 'C';
                            S(i).G = round(1/ptemp) - 1;
                            C(cluster).xd = S(i).xd;
                            C(cluster).yd = S(i).yd;
