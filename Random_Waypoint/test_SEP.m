function test_SEP(s_mobility,s_input)
...
a = 1;                                 %高能节点能量倍数(1+a)
p = 0.1;                               %LEACH的选取概率
pnrm = p/(1+a*m);                      % 普通节点的簇头概率
padv = p/(1+a*m)*(1+a);                % 高级节点的簇头概率
...
for time = 1 : times
    ...
    for i = 1:1:n                          %每个点的坐标及其初始化
        ...
        if i <= m*n
            S(i).E=Eo*(1+a);                     % 初始能量
            S(i).ENERGY = 1;               % 标记是否为高级节点，是高级节点，ENERGY = 1
        end
        if i > m*n
            S(i).E = Eo;
            S(i).ENERGY = 0;
    ...
    for timeIndex = 1:1:length(v_t)
        ...
        %普通节点的选举周期
        if mod(timeIndex, round(1/pnrm)) == 0
            for i = 1:n
                if S(i).ENERGY == 0
                    S(i).G=0;
                end
        ...
        % 高级节点的选举周期
        if mod(timeIndex, round(1/padv)) == 0
            for i = 1:n
                if S(i).ENERGY ==1
                    S(i).G = 0;
                end
        ...
        if(mod(t_HB_period,10) == 0) %10ms一轮选举一轮
            %准备计算簇头数量
            for i = 1:1:n
                %重新选举数据清洗
                S(i).type = 'N';
                ...
                if(S(i).E > 0)                     %节点还有能量
                    temp_rand = rand;              %随机种子
                    if((S(i).G) <= 0)
                        if(S(i).ENERGY == 0 && temp_rand <= (pnrm/(1-pnrm*mod(timeIndex,round(1/pnrm)))))  %普通节点选择成为簇头
                            STATISTICS(timeIndex).CHNUMS = STATISTICS(timeIndex).CHNUMS + 1;
                            cluster = STATISTICS(timeIndex).CHNUMS;
                            S(i).type = 'C';
                            S(i).G = round(1/pnrm) - 1;
                            C(cluster).xd = S(i).xd;
                            C(cluster).yd = S(i).yd;
                        ...
                        if(S(i).ENERGY == 1 && temp_rand <= (padv/(1-padv*mod(timeIndex,round(1/padv)))))  %普通节点选择成为簇头
                            STATISTICS(timeIndex).CHNUMS = STATISTICS(timeIndex).CHNUMS + 1;
                            cluster = STATISTICS(timeIndex).CHNUMS;
                            S(i).type = 'C';
                            S(i).G = round(1/padv) - 1;
                            C(cluster).xd = S(i).xd;
                            C(cluster).yd = S(i).yd;
                            set(vh_node_pos(i),'XData',S(i).xd,'YData',S(i).yd,'Marker','*','Color','green');
                            distance = sqrt((S(i).xd - S(n+1).xd)^2 + (S(i).yd - S(n+1).yd)^2);  %簇头与BS的距离
            ...
            for i = 1:1:n
                ...
                e_use = e_use + (Eo*(1+(S(i).ENERGY)* a) - S(i).E);
